...
for filename in os.listdir(input_dir):
    if not filename.endswith('jsonlines'): continue
    if 'dev' not in filename: continue
    # if filename != 'on.singleton.dev.jsonlines':
    #     continue
    ...
    with io.open(input_dir+os.sep+f_name, encoding='utf-8') as f:
        ...
        for example_num, line in tqdm(enumerate(lines), desc=f_name):
            ...
            with io.open(out_path+os.sep+docname+'.tsv', 'w') as f:
                f.write(text)
